...
function inicializa_mapa(datos) {
	...
	var options = {
		...
		var colorLayer = new L.BarChartDataLayer(datos,options);
                ...
                //Obtenemos la distancia más cercana a una latitud y longitud
                var latlng= new L.LatLng("43.54261","-5.70472");//Simulamos el punto de la bolera del Llauredal
                var latLngs = [];
                var i=0;
                for (var key in colorLayer._layers) {
                    latLngs[i]=colorLayer._layers[key]._latlng;
                    i++;
                }
                console.log(L.GeometryUtil.closest(map,latLngs,latlng));
